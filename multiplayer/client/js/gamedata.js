 ...
 * the very same numbers, colours and card lists: a strategy that changes
 * in the game changes on the rules page in the same deploy, and a family
 * colour can never mean one thing at the table and another in the
 * published rulebook.
 *
 * Loaded before js/preview-app.js, which picks each table straight off
 * window. Plain <script> (no module), because rules.html is a static page
 * with no bundler behind it.
 * ================================================================ */
(function () {
  "use strict";
  ...
  // poster icons are; `n` is counted off the real deck lists, and `key` is the
  ...
  //   a separate ocean strip. So an ocean strategy shows the picture of ITS
  //   OWN ocean card instead (artUid in the data: Kelp Forest shows the Kelp
  //   Forest, Play Again the Arctic Ocean). The Coral Reef card is only the
  //   fallback for a custom plan built out of ocean cards.
  ...
  // Coral Reef: the ocean card a custom plan made of ocean cards falls back to.
  ...
  // run off one board, which is 120 pairs for 16 cores. Writing 110 more by hand
  ...
  //   cost no download, and so a card added to a core turns up in all fifteen
  //   of that core's combos in the same deploy.
  ...
  (function generateComboPairs() {
    ...
    for (let a = 0; a < cores.length; a++) {
      for (let b = a + 1; b < cores.length; b++) {
        ...
        } else if (famA === "ocean" && famB === "ocean") {
          // Two ocean plans. Oceans are not spots on a board, each one IS a
          // new base, so the zone sentences below ("a race for room") would be
          // wrong here. Say which ocean cards count instead.
          const names = [];
          for (const c of aCards.concat(bCards)) if (names.indexOf(c.name) === -1) names.push(c.name);
          const listed = names.length <= 4
            ? (names.length === 1 ? names[0] : names.slice(0, -1).join(", ") + " and " + names[names.length - 1])
            : nameList(names);
          blurb = "Two ocean plans off one board: lay oceans for " + A.label + " and for " + B.label
            + ", and every ocean you play counts toward one of them.";
          bridgeStep = "Only lay oceans that count for one of the two: " + listed + ".";
          bridgeTip = "Two ocean plans need twice the oceans, so pick up the ones you need from the pool whenever you see them.";
        } else if (famA === "ocean" || famB === "ocean") {
          // An ocean plan and an animal plan: the ocean half decides which
          // oceans go down, the animal half decides what is placed on them.
          const O = famA === "ocean" ? A : B, N = famA === "ocean" ? B : A;
          const famN = famA === "ocean" ? famB : famA;
          blurb = "The oceans come from " + O.label + ", the animals on them come from " + N.label
            + " (" + zoneWords(famN) + "), so the two halves never compete for a card.";
          bridgeStep = "Lay your " + O.label + " oceans first, then put your " + N.label + " cards on them.";
          bridgeTip = "Every animal needs an ocean under it, so the ocean half comes first.";
        } else if (!clash) {
  ...
  //   [{ key, label, cardUid }] for an ocean card, since Oceans have no symbol.
  ...
  function stratSymbols(i, strat) {
    ...
    // An ocean strategy wears the picture of its own ocean card. Two ocean
    // plans in one combo are two different pictures, so they are told apart
    // by card, not by family.
    const pushPlan = (plan, fam) => {
      if (plan && !plan.custom && plan.artUid != null) {
        if (!out.some(e => e.cardUid === plan.artUid)) out.push({ key: "ocean", label: plan.label, cardUid: plan.artUid });
      } else {
        push(fam);
      }
    };
    ...
    if (pair) {
      for (const lbl of pair) {
        ...
        if (j >= 0) pushPlan(BUILTIN_STRATEGIES[j], _STRAT_PRIMARY[j]);
      }
    }
    if (!out.length && !custom) pushPlan(s, _STRAT_PRIMARY[i]);
